...
namespace Movestack
{
    ...
    static inline Square Atk_King[3];  // Current moves for current King
    static inline Square Atk_EKing[3]; // Current enemy king attacked squares

    static inline map Check_Status[3]; // When a pawn or a knight does check we can assume at least one check. And only one (initially) since a pawn or knight cannot do discovery
}
...
namespace Movelist
{
    ...
    _ForceInline void InitStack(Board &brd)
    {
        ...
        Movestack::Atk_King[2] = Lookup::King(SquareOf(King<white>(brd)));
        Movestack::Atk_EKing[2] = Lookup::King(SquareOf(King<enemy>(brd)));
        ...
        // Calculate Check from enemy pawns
        {
            ...
            if (pl & King<white>(brd))
                Movestack::Check_Status[2] = Pawn_AttackRight<white>(King<white>(brd));
            else if (pr & King<white>(brd))
                Movestack::Check_Status[2] = Pawn_AttackLeft<white>(King<white>(brd));
            else
                Movestack::Check_Status[2] = 0xffffffffffffffffull;
        }
        ...
        // Calculate Check from enemy knights
        {
            ...
            if (knightcheck)
                Movestack::Check_Status[2] = knightcheck;
        }
    ...
    _Compiletime map ConstCheckmask(const map checkmask)
    {
        if constexpr (inCheck)
            ...
            return 0xffffffffffffffffull; // Helps the compiler to remove unnecessary &
    }
    ...
    {
        if constexpr (inCheck)
            return EnemyOrEmpty<white>(brd) & checkmask;
        else
            return EnemyOrEmpty<white>(brd);
    }

    template <class BoardStatus status, bool justcount, class Callback_Move, int depth, bool inCheck>
    _ForceInline void _enumerate(const Board &brd, map kingatk, const map kingban, const map CheckMask)
    {
        constexpr bool white = status.WhiteMove;
        uint64_t movecnt = 0;
        ...
        // Kingmoves
        {
            ...
            else
            {
                Bitloop(kingatk)
                {
                    ...
                    Movestack::Atk_EKing[1] = Lookup::King(sq);
                    Callback_Move::template Kingmove<status>(brd, King<white>(brd), 1ull << sq);
                }
            }
            ...
            if constexpr (!inCheck && status.CanCastleLeft())
            {
                if (status.CanCastleLeft(kingban, brd.Occ, Rooks<white>(brd)))
                {
                    ...
                    {
                        Movestack::Atk_EKing[1] = Lookup::King(SquareOf(King<white>(brd) << 2));
                        Callback_Move::template KingCastle<status>(brd, (King<white>(brd) | King<white>(brd) << 2), status.Castle_RookswitchL());
                    }
                }
            }
            if constexpr (!inCheck && status.CanCastleRight())
            {
                if (status.CanCastleRight(kingban, brd.Occ, Rooks<white>(brd)))
                {
                    ...
                    {
                        Movestack::Atk_EKing[1] = Lookup::King(SquareOf(King<white>(brd) >> 2));
                        Callback_Move::template KingCastle<status>(brd, (King<white>(brd) | King<white>(brd) >> 2), status.Castle_RookswitchR());
                    }
                }
            }
...

        {
            ...
            if constexpr (status.HasEPPawn)
            {
                ...
                if (EPLpawn | EPRpawn) // Todo: bench if slower or faster
                {
                    ...
                    {
                        if (EPLpawn)
                            Callback_Move::template PawnEnpassantTake<status>(brd, EPLpawn, EPLpawn << 1, Pawn_AttackLeft<white>(EPLpawn));
                        if (EPRpawn)
                            Callback_Move::template PawnEnpassantTake<status>(brd, EPRpawn, EPRpawn >> 1, Pawn_AttackRight<white>(EPRpawn));
                    }
                }
            }
            ...
            if ((Lpawns | Rpawns | Fpawns) & Pawns_LastRank<white>())
            {
                ...
                else
                {
                    while (Promote_Left)
                    {
                        ...
                        Callback_Move::template Pawnpromote<status>(brd, pos, Pawn_AttackLeft<white>(pos));
                    }
                    while (Promote_Right)
                    {
                        const Bit pos = PopBit(Promote_Right);
                        Callback_Move::template Pawnpromote<status>(brd, pos, Pawn_AttackRight<white>(pos));
                    }
                    while (Promote_Move)
                    {
                        const Bit pos = PopBit(Promote_Move);
                        Callback_Move::template Pawnpromote<status>(brd, pos, Pawn_Forward<white>(pos));
                    }
                    while (NoPromote_Left)
                    {
                        const Bit pos = PopBit(NoPromote_Left);
                        Callback_Move::template Pawnatk<status>(brd, pos, Pawn_AttackLeft<white>(pos));
                    }
                    while (NoPromote_Right)
                    {
                        const Bit pos = PopBit(NoPromote_Right);
                        Callback_Move::template Pawnatk<status>(brd, pos, Pawn_AttackRight<white>(pos));
                    }
                    while (NoPromote_Move)
                    {
                        const Bit pos = PopBit(NoPromote_Move);
                        Callback_Move::template Pawnmove<status>(brd, pos, Pawn_Forward<white>(pos));
                    }
                    while (Ppawns)
                    {
                        const Bit pos = PopBit(Ppawns);
                        Callback_Move::template Pawnpush<status>(brd, pos, Pawn_Forward2<white>(pos));
                    }
                }
            }
            else
            ...
            Bitloop(knights)
            {
                ...
                else
                {
                    while (move)
                    {
                        ...
                        Callback_Move::template Knightmove<status>(brd, 1ull << sq, to);
                    }
                }
            }
        }
        ...
        // Bishopmoves
        {
            ...
            Bitloop(bish_pinned)
            {
                ...
                else
                {
                    ...
                    if (pos & queens)
                    {
                        ...
                        {
                            const Bit to = PopBit(move);
                            Callback_Move::template Queenmove<status>(brd, pos, to);
                        }
                    }
                    else
                    {
                        ...
                        {
                            const Bit to = PopBit(move);
                            Callback_Move::template Bishopmove<status>(brd, pos, to);
                        }
                    }
                }
            }
            Bitloop(bish_nopin)
            {
                ...
                else
                {
                    ...
                    {
                        const Bit to = PopBit(move);
                        Callback_Move::template Bishopmove<status>(brd, 1ull << sq, to);
                    }
                }
            }
        }
        ...
        // Rookmoves
        {
            ...
            Bitloop(rook_pinned)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        {
                            const Bit to = PopBit(move);
                            Callback_Move::template Rookmove<status>(brd, pos, to);
                        }
                    }
                }
            }
            Bitloop(rook_nopin)
            {
                ...
                else
                {
                    ...
                    {
                        const Bit to = PopBit(move);
                        Callback_Move::template Rookmove<status>(brd, 1ull << sq, to);
                    }
                }
            }
        }
        // Calculate Enemy Queen
        {
            ...
            Bitloop(queens)
            {
                ...
                else
                {
                    ...
                    {
                        const Bit to = PopBit(move);
                        Callback_Move::template Queenmove<status>(brd, 1ull << sq, to);
                    }
                }
            }
        }
        ...
        {
            if (movecnt == 0)
                Callback_Move.mated = Callback_Move.move_infos.size() - 1;
        }
    }

    template <class BoardStatus status, class Callback_Move>
    _NoInline void EnumerateMoves(Board &brd) // This cannot be forceinline or even inline as its the main recursion entry point
    {
        ...
        map checkmask = Movestack::Check_Status[2];
        map kingban = Movestack::Atk_King[1] = Movestack::Atk_EKing[2];
        map kingatk = Refresh<status, 2>(brd, kingban, checkmask);

        if (checkmask == 0xffffffffffffffffull)
        {
            _enumerate<status, false, Callback_Move, 2, false>(brd, kingatk, kingban, checkmask);
        }
        else if (checkmask != 0)
        {
            _enumerate<status, false, Callback_Move, 2, true>(brd, kingatk, kingban, checkmask);
        }
        else
        {
            Bitloop(kingatk)
            {
                const Square sq = SquareOf(kingatk);
                Movestack::Atk_EKing[1] = Lookup::King(sq);
                Callback_Move::template Kingmove<status>(brd, King<status.WhiteMove>(brd), 1ull << sq);
            }
        }
    }

    template <class BoardStatus status, class Callback_Move>
    _ForceInline void count(Board &brd)
    {
        ...
        {
            if (checkmask != 0)
                _enumerate<status, true, Callback_Move, 1, true>(brd, kingatk, kingban, checkmask); // one check
            else if (kingatk == 0)
                Callback_Move.mated = Callback_Move.move_infos.size() - 1; // double check, no moves
        }         
    }
};
