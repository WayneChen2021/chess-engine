...
public:
    ...
    constexpr BoardStatus(bool white, bool ep, bool wcast_left, bool wcast_right, bool bcast_left, bool bcast_right) : WhiteMove(white), HasEPPawn(ep), WCastleL(wcast_left), WCastleR(wcast_right), BCastleL(bcast_left), BCastleR(bcast_right)
    {
    }

    constexpr BoardStatus() : WhiteMove(true), HasEPPawn(true), WCastleL(true), WCastleR(true), BCastleL(true), BCastleR(true)
    {
    }

    constexpr BoardStatus(int pat) : WhiteMove((pat & 0b100000) != 0), HasEPPawn((pat & 0b010000) != 0),
    ...
    {
        if (WhiteMove)
            return WCastleL | WCastleR;
        else
            return BCastleL | BCastleR;
    }
    ...
    {
        if (WhiteMove)
            return 0b00000101ull;
        else
            return 0b00000101ull << 56;
    }
    constexpr uint64_t Castle_RookswitchL() const
    {
        if (WhiteMove)
            return 0b10010000ull;
        else
            return 0b10010000ull << 56;
    }
    ...
    _Inline constexpr bool CanCastleLeft(map attacked, map occupied, map rook) const
    {
        ...
        {
            if (occupied & WNotOccupiedL)
                return false;
            if (attacked & WNotAttackedL)
                return false;
            if (rook & WRookL)
                return true;
            return false;
        ...
        {
            if (occupied & BNotOccupiedL)
                return false;
            if (attacked & BNotAttackedL)
                return false;
            if (rook & BRookL)
                return true;
            return false;
    ...
    _Inline constexpr bool CanCastleRight(map attacked, map occupied, map rook) const
    {
        ...
        {
            if (occupied & WNotOccupiedR)
                return false;
            if (attacked & WNotAttackedR)
                return false;
            if (rook & WRookR)
                return true;
            return false;
        ...
        {
            if (occupied & BNotOccupiedR)
                return false;
            if (attacked & BNotAttackedR)
                return false;
            if (rook & BRookR)
                return true;
            return false;
...
struct FEN
{
    ...
    static constexpr bool FenInfo(std::string_view FEN)
    {
        ...
        if (EorMinus != '-')
        {
            if (wb == 'w')
            {
                // Todo where to store Enpassant
                // int EPpos = 32 + ('h' - EorMinus);
                if constexpr (field == FenField::hasEP)
                    return true;
            }
            if (wb == 'b')
            {
    ...
    static constexpr uint64_t FenToBmp(std::string_view FEN, char p)
    {
        ...
        while ((c = FEN[i++]) != ' ')
        {
            ...
            default:
                if (c == p)
                    result |= P; // constexpr parsing happens here
            }
            Field--;
        }
        return result;
...
struct Board
{
    ...
    constexpr Board(
        ...
        map wp, map wn, map wb, map wr, map wq, map wk) : BPawn(bp), BKnight(bn), BBishop(bb), BRook(br), BQueen(bq), BKing(bk),
    ...
    constexpr Board(std::string_view FEN) : Board(FEN::FenToBmp(FEN, 'p'), FEN::FenToBmp(FEN, 'n'), FEN::FenToBmp(FEN, 'b'), FEN::FenToBmp(FEN, 'r'), FEN::FenToBmp(FEN, 'q'), FEN::FenToBmp(FEN, 'k'),
                                                  FEN::FenToBmp(FEN, 'P'), FEN::FenToBmp(FEN, 'N'), FEN::FenToBmp(FEN, 'B'), FEN::FenToBmp(FEN, 'R'), FEN::FenToBmp(FEN, 'Q'), FEN::FenToBmp(FEN, 'K'))
    {
    }

    constexpr Board() : BPawn(0), BKnight(0), BBishop(0), BRook(0), BQueen(0), BKing(0),
    ...
    _Compiletime Board MovePromote(const Board &existing, uint64_t from, uint64_t to)
    {
        ...
        if constexpr (IsWhite)
        {
            if constexpr (BoardPiece::Queen == piece)
                return Board(bp & rem, bn & rem, bb & rem, br & rem, bq & rem, bk, wp ^ from, wn, wb, wr, wq ^ to, wk);
            if constexpr (BoardPiece::Rook == piece)
                return Board(bp & rem, bn & rem, bb & rem, br & rem, bq & rem, bk, wp ^ from, wn, wb, wr ^ to, wq, wk);
            if constexpr (BoardPiece::Bishop == piece)
                return Board(bp & rem, bn & rem, bb & rem, br & rem, bq & rem, bk, wp ^ from, wn, wb ^ to, wr, wq, wk);
            if constexpr (BoardPiece::Knight == piece)
                return Board(bp & rem, bn & rem, bb & rem, br & rem, bq & rem, bk, wp ^ from, wn ^ to, wb, wr, wq, wk);
        }
        else
        {
            if constexpr (BoardPiece::Queen == piece)
                return Board(bp ^ from, bn, bb, br, bq ^ to, bk, wp & rem, wn & rem, wb & rem, wr & rem, wq & rem, wk);
            if constexpr (BoardPiece::Rook == piece)
                return Board(bp ^ from, bn, bb, br ^ to, bq, bk, wp & rem, wn & rem, wb & rem, wr & rem, wq & rem, wk);
            if constexpr (BoardPiece::Bishop == piece)
                return Board(bp ^ from, bn, bb ^ to, br, bq, bk, wp & rem, wn & rem, wb & rem, wr & rem, wq & rem, wk);
            if constexpr (BoardPiece::Knight == piece)
                return Board(bp ^ from, bn ^ to, bb, br, bq, bk, wp & rem, wn & rem, wb & rem, wr & rem, wq & rem, wk);
        }
    }
    ...
    {
        if (IsTaking)
            return Move<piece, IsWhite, true>(existing, from, to);
        else
            return Move<piece, IsWhite, false>(existing, from, to);
    }
        ...
        if constexpr (IsTaking)
        {
            ...
            if constexpr (IsWhite)
            {
                ...
                if constexpr (BoardPiece::Pawn == piece)
                    return Board(bp & rem, bn & rem, bb & rem, br & rem, bq & rem, bk, wp ^ mov, wn, wb, wr, wq, wk);
                if constexpr (BoardPiece::Knight == piece)
                    return Board(bp & rem, bn & rem, bb & rem, br & rem, bq & rem, bk, wp, wn ^ mov, wb, wr, wq, wk);
                if constexpr (BoardPiece::Bishop == piece)
                    return Board(bp & rem, bn & rem, bb & rem, br & rem, bq & rem, bk, wp, wn, wb ^ mov, wr, wq, wk);
                if constexpr (BoardPiece::Rook == piece)
                    return Board(bp & rem, bn & rem, bb & rem, br & rem, bq & rem, bk, wp, wn, wb, wr ^ mov, wq, wk);
                if constexpr (BoardPiece::Queen == piece)
                    return Board(bp & rem, bn & rem, bb & rem, br & rem, bq & rem, bk, wp, wn, wb, wr, wq ^ mov, wk);
                if constexpr (BoardPiece::King == piece)
                    return Board(bp & rem, bn & rem, bb & rem, br & rem, bq & rem, bk, wp, wn, wb, wr, wq, wk ^ mov);
            }
            else
            {
                ...
                if constexpr (BoardPiece::Pawn == piece)
                    return Board(bp ^ mov, bn, bb, br, bq, bk, wp & rem, wn & rem, wb & rem, wr & rem, wq & rem, wk);
                if constexpr (BoardPiece::Knight == piece)
                    return Board(bp, bn ^ mov, bb, br, bq, bk, wp & rem, wn & rem, wb & rem, wr & rem, wq & rem, wk);
                if constexpr (BoardPiece::Bishop == piece)
                    return Board(bp, bn, bb ^ mov, br, bq, bk, wp & rem, wn & rem, wb & rem, wr & rem, wq & rem, wk);
                if constexpr (BoardPiece::Rook == piece)
                    return Board(bp, bn, bb, br ^ mov, bq, bk, wp & rem, wn & rem, wb & rem, wr & rem, wq & rem, wk);
                if constexpr (BoardPiece::Queen == piece)
                    return Board(bp, bn, bb, br, bq ^ mov, bk, wp & rem, wn & rem, wb & rem, wr & rem, wq & rem, wk);
                if constexpr (BoardPiece::King == piece)
                    return Board(bp, bn, bb, br, bq, bk ^ mov, wp & rem, wn & rem, wb & rem, wr & rem, wq & rem, wk);
            }
        }
        else
        {
...
#define PositionToTemplate(func)                                 \
    ...
    {                                                            \
        ...
        if (WH && EP && WL && WR && BL && BR)                    \
            return func<BoardStatus(0b111111)>(pos, brd, depth); \
        if (WH && EP && WL && WR && BL && !BR)                   \
            return func<BoardStatus(0b111110)>(pos, brd, depth); \
        if (WH && EP && WL && WR && !BL && BR)                   \
            return func<BoardStatus(0b111101)>(pos, brd, depth); \
        if (WH && EP && WL && WR && !BL && !BR)                  \
            return func<BoardStatus(0b111100)>(pos, brd, depth); \
        if (WH && EP && WL && !WR && BL && BR)                   \
            return func<BoardStatus(0b111011)>(pos, brd, depth); \
        if (WH && EP && WL && !WR && BL && !BR)                  \
            return func<BoardStatus(0b111010)>(pos, brd, depth); \
        if (WH && EP && WL && !WR && !BL && BR)                  \
            return func<BoardStatus(0b111001)>(pos, brd, depth); \
        if (WH && EP && WL && !WR && !BL && !BR)                 \
            return func<BoardStatus(0b111000)>(pos, brd, depth); \
        if (WH && EP && !WL && WR && BL && BR)                   \
            return func<BoardStatus(0b110111)>(pos, brd, depth); \
        if (WH && EP && !WL && WR && BL && !BR)                  \
            return func<BoardStatus(0b110110)>(pos, brd, depth); \
        if (WH && EP && !WL && WR && !BL && BR)                  \
            return func<BoardStatus(0b110101)>(pos, brd, depth); \
        if (WH && EP && !WL && WR && !BL && !BR)                 \
            return func<BoardStatus(0b110100)>(pos, brd, depth); \
        if (WH && EP && !WL && !WR && BL && BR)                  \
            return func<BoardStatus(0b110011)>(pos, brd, depth); \
        if (WH && EP && !WL && !WR && BL && !BR)                 \
            return func<BoardStatus(0b110010)>(pos, brd, depth); \
        if (WH && EP && !WL && !WR && !BL && BR)                 \
            return func<BoardStatus(0b110001)>(pos, brd, depth); \
        if (WH && EP && !WL && !WR && !BL && !BR)                \
            return func<BoardStatus(0b110000)>(pos, brd, depth); \
        if (WH && !EP && WL && WR && BL && BR)                   \
            return func<BoardStatus(0b101111)>(pos, brd, depth); \
        if (WH && !EP && WL && WR && BL && !BR)                  \
            return func<BoardStatus(0b101110)>(pos, brd, depth); \
        if (WH && !EP && WL && WR && !BL && BR)                  \
            return func<BoardStatus(0b101101)>(pos, brd, depth); \
        if (WH && !EP && WL && WR && !BL && !BR)                 \
            return func<BoardStatus(0b101100)>(pos, brd, depth); \
        if (WH && !EP && WL && !WR && BL && BR)                  \
            return func<BoardStatus(0b101011)>(pos, brd, depth); \
        if (WH && !EP && WL && !WR && BL && !BR)                 \
            return func<BoardStatus(0b101010)>(pos, brd, depth); \
        if (WH && !EP && WL && !WR && !BL && BR)                 \
            return func<BoardStatus(0b101001)>(pos, brd, depth); \
        if (WH && !EP && WL && !WR && !BL && !BR)                \
            return func<BoardStatus(0b101000)>(pos, brd, depth); \
        if (WH && !EP && !WL && WR && BL && BR)                  \
            return func<BoardStatus(0b100111)>(pos, brd, depth); \
        if (WH && !EP && !WL && WR && BL && !BR)                 \
            return func<BoardStatus(0b100110)>(pos, brd, depth); \
        if (WH && !EP && !WL && WR && !BL && BR)                 \
            return func<BoardStatus(0b100101)>(pos, brd, depth); \
        if (WH && !EP && !WL && WR && !BL && !BR)                \
            return func<BoardStatus(0b100100)>(pos, brd, depth); \
        if (WH && !EP && !WL && !WR && BL && BR)                 \
            return func<BoardStatus(0b100011)>(pos, brd, depth); \
        if (WH && !EP && !WL && !WR && BL && !BR)                \
            return func<BoardStatus(0b100010)>(pos, brd, depth); \
        if (WH && !EP && !WL && !WR && !BL && BR)                \
            return func<BoardStatus(0b100001)>(pos, brd, depth); \
        if (WH && !EP && !WL && !WR && !BL && !BR)               \
            return func<BoardStatus(0b100000)>(pos, brd, depth); \
        if (!WH && EP && WL && WR && BL && BR)                   \
            return func<BoardStatus(0b011111)>(pos, brd, depth); \
        if (!WH && EP && WL && WR && BL && !BR)                  \
            return func<BoardStatus(0b011110)>(pos, brd, depth); \
        if (!WH && EP && WL && WR && !BL && BR)                  \
            return func<BoardStatus(0b011101)>(pos, brd, depth); \
        if (!WH && EP && WL && WR && !BL && !BR)                 \
            return func<BoardStatus(0b011100)>(pos, brd, depth); \
        if (!WH && EP && WL && !WR && BL && BR)                  \
            return func<BoardStatus(0b011011)>(pos, brd, depth); \
        if (!WH && EP && WL && !WR && BL && !BR)                 \
            return func<BoardStatus(0b011010)>(pos, brd, depth); \
        if (!WH && EP && WL && !WR && !BL && BR)                 \
            return func<BoardStatus(0b011001)>(pos, brd, depth); \
        if (!WH && EP && WL && !WR && !BL && !BR)                \
            return func<BoardStatus(0b011000)>(pos, brd, depth); \
        if (!WH && EP && !WL && WR && BL && BR)                  \
            return func<BoardStatus(0b010111)>(pos, brd, depth); \
        if (!WH && EP && !WL && WR && BL && !BR)                 \
            return func<BoardStatus(0b010110)>(pos, brd, depth); \
        if (!WH && EP && !WL && WR && !BL && BR)                 \
            return func<BoardStatus(0b010101)>(pos, brd, depth); \
        if (!WH && EP && !WL && WR && !BL && !BR)                \
            return func<BoardStatus(0b010100)>(pos, brd, depth); \
        if (!WH && EP && !WL && !WR && BL && BR)                 \
            return func<BoardStatus(0b010011)>(pos, brd, depth); \
        if (!WH && EP && !WL && !WR && BL && !BR)                \
            return func<BoardStatus(0b010010)>(pos, brd, depth); \
        if (!WH && EP && !WL && !WR && !BL && BR)                \
            return func<BoardStatus(0b010001)>(pos, brd, depth); \
        if (!WH && EP && !WL && !WR && !BL && !BR)               \
            return func<BoardStatus(0b010000)>(pos, brd, depth); \
        if (!WH && !EP && WL && WR && BL && BR)                  \
            return func<BoardStatus(0b001111)>(pos, brd, depth); \
        if (!WH && !EP && WL && WR && BL && !BR)                 \
            return func<BoardStatus(0b001110)>(pos, brd, depth); \
        if (!WH && !EP && WL && WR && !BL && BR)                 \
            return func<BoardStatus(0b001101)>(pos, brd, depth); \
        if (!WH && !EP && WL && WR && !BL && !BR)                \
            return func<BoardStatus(0b001100)>(pos, brd, depth); \
        if (!WH && !EP && WL && !WR && BL && BR)                 \
            return func<BoardStatus(0b001011)>(pos, brd, depth); \
        if (!WH && !EP && WL && !WR && BL && !BR)                \
            return func<BoardStatus(0b001010)>(pos, brd, depth); \
        if (!WH && !EP && WL && !WR && !BL && BR)                \
            return func<BoardStatus(0b001001)>(pos, brd, depth); \
        if (!WH && !EP && WL && !WR && !BL && !BR)               \
            return func<BoardStatus(0b001000)>(pos, brd, depth); \
        if (!WH && !EP && !WL && WR && BL && BR)                 \
            return func<BoardStatus(0b000111)>(pos, brd, depth); \
        if (!WH && !EP && !WL && WR && BL && !BR)                \
            return func<BoardStatus(0b000110)>(pos, brd, depth); \
        if (!WH && !EP && !WL && WR && !BL && BR)                \
            return func<BoardStatus(0b000101)>(pos, brd, depth); \
        if (!WH && !EP && !WL && WR && !BL && !BR)               \
            return func<BoardStatus(0b000100)>(pos, brd, depth); \
        if (!WH && !EP && !WL && !WR && BL && BR)                \
            return func<BoardStatus(0b000011)>(pos, brd, depth); \
        if (!WH && !EP && !WL && !WR && BL && !BR)               \
            return func<BoardStatus(0b000010)>(pos, brd, depth); \
        if (!WH && !EP && !WL && !WR && !BL && BR)               \
            return func<BoardStatus(0b000001)>(pos, brd, depth); \
        if (!WH && !EP && !WL && !WR && !BL && !BR)              \
            return func<BoardStatus(0b000000)>(pos, brd, depth); \
