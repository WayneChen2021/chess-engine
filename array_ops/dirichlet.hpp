...
// Source code: https://github.com/gcant/dirichlet-cpp
...
std::vector<double> basic_distr(double a, uint64_t size)
{
  std::random_device rd;
  std::mt19937 gen(rd());
  dirichlet_distribution<std::mt19937> d(std::vector<double>(size, a));
  return d(gen);
}